...
void operm_work()
{
 ...
 for(i=0;i<nkperm;i++){
   if(verbose){
     printf(" kperm = %d  i = %d: ",kperm(fperms[i]->data,kcov,0),i);
  ...
  */
 if(verbose){
   printf("Testing f(i,nkperm,kperm):\n");
 }
 for(i=0;i<nkperm;i++){
   for(j = 0;j<nkperm;j++){
     printf("  f(%d,",i);
     gsl_permutation_fprintf(stdout,fperms[j],"%u ");
     printf(") = %12.6f\n",f(i,fperms[j]->data,kcov,0,nkperm));
   }
 }
     ...
     for(k=0;k<noperm;k++){
       ...
       for(off1=0;off1<1;off1++){
         for(off2=1;off2<kcov;off2++){
           fi = f(i,operms[k]->data,kcov,0,nkperm);
           fj = f(j,operms[k]->data,kcov,off2,nkperm);
           c[i][j] += fi*fj;
	 }
       }
     }
