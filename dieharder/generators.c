...
void initialize_generators()
{
 ...
 /*
  * Allocate all the (now standardized) rngs known directly
  * to dieharder via the GSL (variable list not under our control)
  * or in source, including known hardware generators and file
  * input "generators".  This routine also sets the counts of
  * each "kind" of generator into global/shared variables.
  */
 ...
 */
int select_generator(int gennum,char *genname,uint initial_seed)
{
