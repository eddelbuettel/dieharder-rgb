...
void Btest_create(Btest *btest, unsigned int npts,
                    char *testname, char *rngname)
{

 int i;
 if(verbose > 10){
   printf("Btest_create(): Creating test struct for %u points.\n",npts);
 }
 btest->x = (double *) malloc(sizeof(double)*npts);       /* sample results */
 btest->y = (double *) malloc(sizeof(double)*npts);       /* expected sample results */
 btest->sigma = (double *) malloc(sizeof(double)*npts);   /* known sample sigma */
...
void Btest_conclusion(Btest *btest)
{
 ...
 if(!quiet){
   ...
   printf("#        %12d   %11.2f %11.5f\n",btest->npts,btest->chisq,btest->pvalue);
